...
class DisenCDR(nn.Module):
    def __init__(self, opt):
        ...
        self.params_dicts.extend(self.share_GNN.params_dicts)
        ...
        for i in range(opt['k']):
            # self.u = nn.Embedding(
            #     opt[f"fname{i}_user_num"], opt["feature_dim"])
            a = nn.Embedding(
                opt[f"fname{i}_user_num"], opt["feature_dim"])
            self.domain_user_embeddings.append(a)
            self.domain_item_embeddings.append(nn.Embedding(
                opt[f"fname{i}_item_num"], opt["feature_dim"]))
            self.domain_user_embeddings_share.append(
                nn.Embedding(opt[f"fname{i}_user_num"], opt["feature_dim"]))

        for i in range(opt['k']):
            self.params_dicts.append(
                {"params": self.domain_item_embeddings[i].parameters()})
            self.params_dicts.append(
                {"params": self.domain_user_embeddings[i].parameters()})
            self.params_dicts.append(
                {"params": self.domain_user_embeddings_share[i].parameters()})
            self.params_dicts.append(
                {"params": self.domain_specific_GNNs[i].parameters()})
            self.params_dicts.append(
                {"params": self.domain_share_GNNs[i].parameters()})
    ...
    def forward(self, UVs, VUs):
        ...
        for i in range(self.opt['k']):
            ...
            domain_users.append(self.domain_user_embeddings[i](
                self.domain_user_indices[i]))
            domain_items.append(self.domain_item_embeddings[i](
                self.domain_item_indices[i]))
            domain_user_shares.append(
                self.domain_user_embeddings_share[i](self.domain_user_indices[i]))
            # print(domain_user_shares[i].shape)
            a, b = self.domain_specific_GNNs[i](
                domain_users[i], domain_items[i], UVs[i], VUs[i])
            ...
            a, b = self.domain_share_GNNs[i].forward_user_share(
                domain_users[i], UVs[i], VUs[i])
        ...
        for i in range(self.opt['k']):
            domain_share_kld = self._kld_gauss(
                mean, sigma, domain_user_means[i], domain_user_sigmas[i])
        ...
        domain_learn_users = []
        for i in range(self.opt['k']):
            domain_learn_users.append(
                user_share + domain_learn_specific_users[i])
        # source_learn_user =
