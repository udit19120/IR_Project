...
def build_data_dic(data):
    ...
    for element in data:
        ...
        dict_add(data_dic, user_id, (item_id, rating))
    return data_dic
...
def split_loo(data_dic, filepath):
    train_file = filepath+'/train_sentiment.txt'
    test_file = filepath+'/test_sentiment.txt'
    train_dic = {}
    test_dic = {}
    train_f = open(train_file, 'w+')
    test_f = open(test_file, 'w+')
    for user_id in data_dic.keys():
        ...
        if len(items_train) > 0:
            ...
            for i in items_train:
                # print('\t'.join(list_str([user_id]+[i[0]]+[i[1]]))+'\n')
                train_f.write(
                    '\t'.join(list_str([user_id]+[i[0]]+[i[1]]))+'\n')
        test_dic[user_id] = items[split_index:]
        # print(items[split_index:])
        test_f.write('\t'.join(list_str(
            [user_id]+[items[split_index:][0][0]]+[items[split_index:][0][1]]))+'\n')
    train_f.close()
    test_f.close()
    return train_dic, test_dic
...
def get_test_neg(train_dic, test_dic, neg_num, filepath):
    test_neg = filepath + '/test_neg_sentiment.txt'
...
for i in ["digital", "cell_phones", "movies"]:
    filepath = i
    # dataset = 'cloth'
